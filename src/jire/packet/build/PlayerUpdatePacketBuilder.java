...
import jire.player.LocalPlayerListEntry;
import jire.player.LocalPlayerListEntry.LocalPlayerStatus;
import jire.player.Player;
import jire.world.ChatMessage;
import jire.world.TranslationHelpers;
import jire.world.UpdateFlagSet;
import jire.world.UpdateFlagSet.UpdateType;

@BuildsPacket(PlayerUpdatePacket.class)
public class PlayerUpdatePacketBuilder implements PacketBuilder {
	...
	public Packet build(PacketRepresentation packetRep) {
		Player player = ((PlayerUpdatePacket) packetRep).getPlayer();
		...
		for (LocalPlayerListEntry entry : player.getLocalPlayerEntries()) {
			...
			if (entry.getStatus() == LocalPlayerStatus.NO_CHANGE) {
				...
				if (updatePlayer.getUpdateFlags().isUpdateRequired()) {
					updatePlayer(updateBlock, updatePlayer,
							PlayerStatus.CURRENT_PLAYER, false);
				}
	...
	private void addNewPlayer(RSOutputStream output, Player ourPlayer,
			Player otherPlayer) {
		...
		int yPos = otherPlayer.getPosition().getY()
				- ourPlayer.getPosition().getY();
		int xPos = otherPlayer.getPosition().getX()
				- ourPlayer.getPosition().getX();

		output.writeBits(5, yPos); // XXX UPDATE: Multiple people on top of each
									// other disappear
		...
		if (player.isTeleporting() || player.isMapRegionChanging()) {
			...
			if (player.getSprites().getPrimarySprite() == -1) {
				...
				if (player.getSprites().getSecondarySprite() == -1) {
					...
					output.writeBits(1,
							isUpdateRequired(player.getUpdateFlags()));
				} else {
					// run
					output.writeBits(1, 1);
					output.writeBits(2, 2);
					output.writeBits(3, player.getSprites().getPrimarySprite());
					output.writeBits(3, player.getSprites()
							.getSecondarySprite());
					output.writeBits(1,
							isUpdateRequired(player.getUpdateFlags()));
				}
	...
	private void updatePlayer(RSOutputStream output, Player player,
		...
		int mask = 0;
		if (flagSet.contains(UpdateType.CHAT)
				&& !player.getChatQueue().isEmpty() && !ourPlayer) {
			mask |= 0x80;
			appendChatMessageUpdate(updateBlock, player, player.getChatQueue()
					.peek());
		}
		if (flagSet.contains(UpdateType.APPEARANCE)
				|| status == PlayerStatus.NEW_PLAYER) {
			mask |= 0x10;
	...
	private void appendPlayerAppearanceUpdate(RSOutputStream output,
			Player player) {
	...
	private void appendChatMessageUpdate(RSOutputStream output, Player player,
			ChatMessage chat) {
		output.writeLEShort((chat.getColor() & 0xFF) << 8 | chat.getEffects()
				& 0xFF);
